                            ...
                            ?>
                                    <tr>
                                        ...
                                        <td class="align-top text-center">
                                            <?php if ($item["StatusYearPlan"] == 'Y'): ?>
                                                <font color='green'>ใช้งาน</font>
                                            <?php elseif ($item["StatusYearPlan"] == 'N'): ?>
                                        ...
                                        <td class="align-top text-center">
                                            <?php if ($item["ActiveStatus"] == 'Y'): ?>
                                                ...
                                                <br />
                                                <span class="detail">
                                                    <a id="btnView" class="btn btn-primary" onclick="window.open('<?= $routeGroup ?>/view/<?= $item["OverAllPlanYear_id"] ?>.<?= $item["GroupUserID"] ?>', '_blank', 'width=1400,height=550,scrollbars=1');">ดูรายละเอียดข้อมูล</a>
                                                </span>
                                            <?php elseif ($item["ActiveStatus"] == 'N' and $item["ReturnName"] != ''): ?>
                                                <a id="btnEdit" class="btn btn-warning" href="<?= $routeGroup ?>/edt/<?= $item["OverAllPlanYear_id"] ?>.<?= $item["GroupUserID"] ?>" title="<?= $item["RemarksReturn"] ?>"><i class="fas fa-edit"></i> ทบทวนข้อมูล</a>
                                            <?php else: ?>
                                                <a id="btnEdit" class="btn btn-warning" href="<?= $routeGroup ?>/edt/<?= $item["OverAllPlanYear_id"] ?>.<?= $item["GroupUserID"] ?>"><i class="fas fa-edit"></i> ปรับปรุงข้อมูล</a>
                                                <a id="btnDel" class="btn btn-danger" href="<?= $routeGroup ?>/del/<?= $item["OverAllPlanYear_id"] ?>.<?= $item["GroupUserID"] ?>"><i class="fas fa-edit"></i> ลบ</a>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                <?php
