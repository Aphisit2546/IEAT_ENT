...
<div class="row">
    <form id="frm" class="form-horizontal" action="<?php echo base_url(); ?>/<?= $routeGroup ?>" method="post">
        ...
        <div class="row text-right">
            <div class="col-12">
                <a id="btnAdd" class="btn btn-success ml-2" href="<?= $routeGroup ?>/add"><i class="fas fa-plus-square"></i> จัดทำรายละเอียดแผน</a>
            </div>
        </div>
    </form>
</div>
                    ...
                    <table class="table table-bordered table-hover">
                        <thead>
                            <tr class="align-middle">
                                ...
                                <th class="align-middle text-center" width="20%">ชื่อแผนปฏิบัติยุทธศาสตร์</th>
                                    ...
                                    <tr>
                                        ...
                                        <td class="align-top text-center">
                                            <?php if ($item["ActiveStatus"] == 'Y'): ?>
                                                <span class="detail">
                                                    <a id="btnView" class="btn btn-primary" onclick="window.open('<?= $routeGroup ?>/view/<?= $item["CreatePlan_id"] ?>', '_blank', 'width=1050,height=550,scrollbars=1');">ดูรายละเอียดข้อมูล</a>
                                                </span>
                                            <?php elseif ($item["ActiveStatusIns"] == 'N' or $item["ActiveStatusApprove"] == 'N'): ?>
                                                <a id="btnEdit" class="btn btn-warning" href="<?= $routeGroup ?>/edt/<?= $item["CreatePlan_id"] ?>" title="<?= $item["Remarks"] ?>"><i class="fas fa-edit"></i> ทบทวนข้อมูล</a>
                                            <?php else: ?>
                                                <a id="btnEdit" class="btn btn-warning" href="<?= $routeGroup ?>/edt/<?= $item["CreatePlan_id"] ?>"><i class="fas fa-edit"></i> ปรับปรุงข้อมูล</a>
                                                <a id="btnDel" class="btn btn-danger" href="<?= $routeGroup ?>/del/<?= $item["CreatePlan_id"] ?>"><i class="fas fa-edit"></i> ลบ</a>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                <?php
