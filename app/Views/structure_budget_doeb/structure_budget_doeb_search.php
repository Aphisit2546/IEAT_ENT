                    ...
                    <table class="table table-bordered table-hover">
                        <thead>
                            <tr class="align-middle">
                                ...
                                <th class="align-middle text-center">รหัสควบคุมเวอร์ชัน</th>
                                <th class="align-middle text-center" width="20%">สถานะข้อมูล</th>
                                <th class="align-middle text-center" width="20%">วัน-เวลาปรับปรุงข้อมูลล่าสุด</th>
                                <th class="align-middle text-center" width="20%">จัดการ</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            ...
                            ?>
                                    <tr>
                                        ...
                                        <!--<td class="align-top text-left"><?= $item["BudgetStructureCode"]; ?></td>-->
                                        <td class="align-top text-left"><?= $item["VersionControl"]; ?></td>
                                        <td class="align-top text-center">
                                            <?php if ($item["ActiveStatus"] == 'Y'):
                                                echo "<font color='green'>ยืนยันข้อมูล</font>";
                                            else:
                                                echo "<font color='red'>บันทึกแบบร่าง</font>";
                                            endif; ?>
                                        </td>
                                        <td class="align-top text-center"><?= $item["ModifyDate"]; ?></td>
                                        <td class="align-top text-center">
                                            <?php if ($item["ActiveStatus"] != 'Y'): ?>
                                                <a id="btnEdit" class="btn btn-warning" href="<?= $routeGroup ?>/edt/<?= $item["BudgetStructure_id"] ?>"><i class="fas fa-edit"></i> ปรับปรุงข้อมูล</a>
                                                <a id="btnDel" class="btn btn-danger" href="<?= $routeGroup ?>/del/<?= $item["BudgetStructure_id"] ?>"><i class="fas fa-edit"></i> ลบ</a>
                                            <?php else: ?>
                                                <span class="detail">
                                                    <a id="btnView" class="btn btn-primary" onclick="window.open('<?= $routeGroup ?>/view/<?= $item["BudgetStructure_id"] ?>', '_blank', 'width=1020,height=550,scrollbars=1');">ดูรายละเอียดข้อมูล</a>
                                                </span>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
