                            ...
                            ?>
                                    <tr>
                                        ...
                                        <td class="align-top text-center">
                                            <?php if ($item["ActiveStatusPlanMonthChange"] == 'Y'): ?>
                                                <a id="btnView" class="btn btn-primary" onclick="window.open('<?= $routeGroup ?>/view2/<?= $item["CreatePlanDetail_id"] ?>', '_blank', 'width=1200,height=550,scrollbars=1');">ดูรายละเอียดข้อมูลปรับแผน ง.</a>
                                            <?php elseif ($item["ActiveStatusChangeALL"] == 'Y'): ?>
                                                <a id="btnEdit" class="btn btn-danger" href="<?= $routeGroup ?>/edt2/<?= $item["CreatePlanDetail_id"] ?>"><i class="fas fa-edit"></i> ปรับแผน ง.</a>
                                            <?php elseif ($item["ActiveStatusChangeTPALL"] == 'Y'): ?>
                                                <a id="btnEdit" class="btn btn-danger" href="<?= $routeGroup ?>/edt2/<?= $item["CreatePlanDetail_id"] ?>"><i class="fas fa-edit"></i> ปรับแผน ต.</a>
                                            <?php elseif ($item["ActiveStatusChangeALLTConfirm"] == 'Y'): ?>
                                                <a id="btnView" class="btn btn-primary" onclick="window.open('<?= $routeGroup ?>/view/<?= $item["CreatePlanDetail_id"] ?>', '_blank', 'width=1200,height=550,scrollbars=1');">ดูรายละเอียดข้อมูล</a>
                                            <?php elseif ($item["ActiveStatusPlanMonth"] == 'Y'): ?>
                                                <a id="btnView" class="btn btn-primary" onclick="window.open('<?= $routeGroup ?>/view/<?= $item["CreatePlanDetail_id"] ?>', '_blank', 'width=1200,height=550,scrollbars=1');">ดูรายละเอียดข้อมูล</a>
                                            <?php else: ?>
                                                <a id="btnEdit" class="btn btn-info" href="<?= $routeGroup ?>/edt/<?= $item["CreatePlanDetail_id"] ?>"><i class="fas fa-edit"></i> เลือก</a>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                <?php
