                            ...
                            ?>
                                    <tr>
                                        ...
                                        <td class="align-top text-center">
                                            <?php if ($item["ActiveStatusFirstALL"] == 'Y'): ?>
                                                <font color='green'><b>ใช้งาน</b></font>
                                                <br />
                                                <span class="detail">
                                                    <a id="btnView" class="btn btn-primary" onclick="window.open('create_budget_control_doeb/view/<?= $item["OverAllBudget_id"] ?>.<?= $item["CreateBudgetControl_id"] ?>', '_blank', 'width=1450,height=600,scrollbars=1');">ดูรายละเอียดข้อมูล</a>
                                                </span>
                                            <?php elseif ($item["ActiveStatusIns"] == 'N'): ?>
                                                <a id="btnEdit" class="btn btn-secondary" href="<?= $routeGroup ?>/edt/<?= $item["OverAllBudget_id"] ?>.<?= $item["CreateBudgetControl_id"] ?>" title="<?= $item["RemarksReturn"] ?>"><i class="fas fa-edit"></i> ตรวจสอบและพิจารณา</a>
                                            <?php else: ?>
                                                <a id="btnEdit" class="btn btn-warning" href="<?= $routeGroup ?>/edt/<?= $item["OverAllBudget_id"] ?>.<?= $item["CreateBudgetControl_id"] ?>"><i class="fas fa-edit"></i> ตรวจสอบและพิจารณา</a>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                <?php
