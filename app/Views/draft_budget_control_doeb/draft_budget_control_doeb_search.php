                            ...
                            ?>
                                    <tr>
                                        ...
                                        <td class="align-top text-center">
                                            <?php if ($item["ActiveStatus_DraftBudgetControl"] == 'Y'): ?>
                                                <font color='green'><b>ใช้งาน</b></font>
                                                <br />
                                                <span class="detail">
                                                    <a id="btnView" class="btn btn-primary" onclick="window.open('<?= $routeGroup ?>/view/<?= $item["DraftBudgetControl_id"] ?>', '_blank', 'width=1450,height=600,scrollbars=1');">ดูรายละเอียดข้อมูล</a>
                                                </span>
                                            <?php elseif ($item["ActiveStatus_DraftBudgetControl"] == 'N' and $item["ReturnName"] != ''): ?>
                                                <a id="btnEdit" class="btn btn-warning" href="<?= $routeGroup ?>/edt/<?= $item["DraftBudgetControl_id"] ?>.0" title="<?= $item["RemarksReturn"] ?>"><i class="fas fa-edit"></i> ทบทวนข้อมูล</a>
                                            <?php elseif ($item["ActiveStatus_DraftBudgetControl"] == 'N' and $item["ReturnName"] == ''): ?>
                                                <a id="btnEdit" class="btn btn-secondary" href="<?= $routeGroup ?>/edt/<?= $item["DraftBudgetControl_id"] ?>.0" title="<?= $item["RemarksReturn"] ?>"><i class="fas fa-edit"></i> ปรับปรุงข้อมูล</a>
                                            <?php else: ?>
                                                <a id="btnEdit" class="btn btn-warning" href="<?= $routeGroup ?>/add/<?= $item["OverAllBudget_id"] ?>"><i class="fas fa-edit"></i> จัดทำร่างคำขอ</a>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                <?php
