...
use App\Models\TypeBudget_doeb;
...
class TypeBudget_doeb_Controller extends BaseController
{
	protected $programCode = "TypeBudget_doeb";
	...
	public function __construct()
	{
		...
		//$this->model = new TypeBudget_doeb();
	...
	public function index()
	{
		...
		$res = $client->request('GET', Env("app.RestBaseURL") . '/api/TypeBudgetDoeb', [
	...
	public function store()
	{
		...
		*/
		$data = null;
		$client = new Client();
		$res = $client->post(Env("app.RestBaseURL") . '/api/TypeBudgetDoeb', [
			"body" => json_encode($input)
		]);
		$data = $res->getBody()->getContents('json');
		$data = json_decode($data, true);
	...
	public function edit($value)
	{
		...
		$client = new Client();
		$res = $client->request('GET', Env("app.RestBaseURL") . '/api/TypeBudgetDoeb/' . $value . '/edit');
		$data = $res->getBody()->getContents('json');
		$data = json_decode($data, true);
	...
	public function update()
	{
		...
		$input = $this->request->getRawInput();
		$client = new Client();
		$res = $client->put(Env("app.RestBaseURL") . '/api/TypeBudgetDoeb/' . $input['TypeBudgetType_id'], [
			"body" => json_encode($input)
		]);
		$data = $res->getBody()->getContents('json');
		$data = json_decode($data, true);
	...
	public function delete($id)
	{
		...
		helper("form");
		$client = new Client();
		$client->delete(Env("app.RestBaseURL") . '/api/TypeBudgetDoeb/' . $id, [
			"body" => json_encode($id)
		]);
