...
class listusesystem_doeb_Controller extends BaseController
{
	protected $programCode = "listusesystem_doeb";
	...
	public function index()
	{
		...
		$res = $client->request('GET', Env("app.RestBaseURL") . '/api/ListusesystemDoeb', [
	...
	public function edit($value)
	{
		...
		$client = new Client();
		$res = $client->request('GET', Env("app.RestBaseURL") . '/api/GroupUserDoeb/' . $value . '/edit');
		$data = $res->getBody()->getContents('json');
		$data = json_decode($data, true);
	...
	public function update()
	{
		...
		$input = $this->request->getRawInput();
		$client = new Client();
		$res = $client->put(Env("app.RestBaseURL") . '/api/GroupUserDoeb/' . $input['UnitManagement_id'], [
			"body" => json_encode($input)
		]);
