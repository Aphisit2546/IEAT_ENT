...
class calendar_management_Controller extends BaseController
{
	protected $programCode = "calendar_management";
	...
	public function __construct()
	{
		...
		$this->model = new calendar_activity();
	...
	public function index()
	{
		...
		foreach ($lstData as $lstData) {
			...
			$final_data[] = array(
				"calendar_activity_id" => $lstData['calendar_activity_id'],
				"code_ref" => $lstData['code_ref'],
				"name_th" => $lstData['name_th'],
				"name_en" => $lstData['name_en'],
				"detail" => $lstData['detail'],
				'type_activity_code' => $lstData['type_activity_code'],
				"start_calendar" => $lstData['start_calendar'],
				"start_calendar_th" => $lstData['start_calendar_th'],
				"finish_calendar" => $lstData['finish_calendar'],
				"finish_calendar_th" => $lstData['finish_calendar_th'],
				"status_calendar" => $lstData['status_calendar'],
	...
	public function store()
	{
		...
		$rules = [
			...
			"name_en" => "required",
			"detail" => "required",
			"start_calendar" => "required",
			"finish_calendar" => "required",
			"status_calendar" => "required",
			"remarks" => "required",
		];
		...
		try {
			...
			$postData = [
				...
				"detail" => $input['detail'],
				"type_activity_code" => $input['type_activity'],
				"start_calendar" => $input['start_calendar'],
				"finish_calendar" => $input['finish_calendar'],
			...
			if ($input['hidMainAction'] == "publish") {
				$postData['status_calendar'] = 'PUBLISH';
			} else {
				$postData['status_calendar'] = 'DRAFT';
			}
	...
	public function update()
	{
		...
		$postData = [
			...
			"start_calendar" => $input['start_calendar'],
			"finish_calendar" => $input['finish_calendar'],
			"status_calendar" => $input['status_calendar'],
			"remarks" => $input['remarks'],
			"update_by" => '1',
			"update_datetime" => Date("Y-m-d H:i:s")
		];
		$this->model->update($input['calendar_activity_id'], $postData);
	...
	public function delete($value)
	{
		// iclude helper form
		helper("form");
		$this->model->delete_calendar_management($value);
		/*
