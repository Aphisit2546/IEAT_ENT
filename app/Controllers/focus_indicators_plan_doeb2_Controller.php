...
class focus_indicators_plan_doeb2_Controller extends BaseController
{
	protected $programCode = "focus_indicators_plan_doeb2";
	...
	public function index()
	{
		...
		$res = $client->request('GET', Env("app.RestBaseURL") . '/api/FocusIndicatorsPlanDoeb2', [
	...
	public function store()
	{
		...
		try {
			...
			// STEP1
			$client = new Client();
			$res = $client->post(Env("app.RestBaseURL") . '/api/FocusIndicatorsPlanDoeb2', [
				"body" => json_encode($input)
			]);
			$data = $res->getBody()->getContents('json');
			$data = json_decode($data, true);
			$StrategyTarGet_id = $data['data'];

			// STEP2
			$res_2 = $client->request('GET', Env("app.RestBaseURL") . '/api/FocusIndicatorsPlanDoeb2/' . $StrategyTarGet_id . '/edit');
	...
	public function edit($value)
	{
		...
		$client = new Client();
		$res = $client->request('GET', Env("app.RestBaseURL") . '/api/FocusIndicatorsPlanDoeb2/' . $value . '/edit');
		$data = $res->getBody()->getContents('json');
		$data = json_decode($data, true);
	...
	public function update()
	{
		...
		$client = new Client();
		$res = $client->put(Env("app.RestBaseURL") . '/api/FocusIndicatorsPlanDoeb2/' . $input['StrategyTarGet_id'], [
			"body" => json_encode($input)
		]);
		$data = $res->getBody()->getContents('json');
		$data = json_decode($data, true);

		$client->delete(Env("app.RestBaseURL") . '/api/FocusIndicatorsPlanDetailDoeb2/' . $input['StrategyTarGet_id'], [
			"body" => json_encode($input['StrategyTarGet_id'])
		]);
		for ($i = 0; $i < count($input['Name']); $i++) {

			if ($input['Name'][$i] != '' && $input['Name'][$i] != null && $input['StrategyTarGet_id'] != '' && $input['StrategyTarGet_id'] != null) {
				$postData_detail = [
					...
					"StrategyTarGet_id" => $input['StrategyTarGet_id']
				];

				$client->post(Env("app.RestBaseURL") . '/api/FocusIndicatorsPlanDetailDoeb2', [
	...
	public function delete($id)
	{
		...
		helper("form");
		$client = new Client();
		$client->delete(Env("app.RestBaseURL") . '/api/FocusIndicatorsPlanDoeb2/' . $id, [
			"body" => json_encode($id)
		]);
