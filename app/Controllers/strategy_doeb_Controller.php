...
use App\Models\strategy_doeb;
use App\Models\FUNCTION_REUSE\dropdown_reuse;
use App\Models\FUNCTION_REUSE\function_reuse;
use Exception;


class strategy_doeb_Controller extends BaseController
{
	protected $programCode = "strategy_doeb";
	...
	public function index()
	{
		...
		foreach ($lstData as $lstData) {
			...
			$final_data[] = array(
				"strategy_doeb_id" => $lstData['strategy_doeb_id'],
				"code_ref" => $lstData['code_ref'],
				"plan_doeb_name" => $lstData['plan_doeb_name'],
				"strategy_head_detail_name" => $lstData['strategy_head_detail_name'],
				"strategy_head_detail_doeb_id" => $lstData['strategy_head_detail_doeb_id'],
	...
	public function add()
	{
		...
		$this->pageData['dropdown_strategy_head_detail_doeb'] = $this->dropdown->dropdown_strategy_head_detail_doeb();
	...
	public function store()
	{
		...
		try {
			...
			$postData = [
				"code_ref" => $code_ref,
				"strategy_head_detail_doeb_id" => $input['strategy_head_detail_doeb_id'],
				"status_strategy" => $status_strategy,
			...
			$strategy_doeb_id = $this->model->getInsertID();

			$count_strategy_name = count($input['strategy_name']);
			for ($a = 0; $a < $count_strategy_name; $a++) {
				$postData_detail = [
					"strategy_doeb_id" => $strategy_doeb_id,
					"strategy_name" => $input['strategy_name'][$a],
				...
				if ($input['strategy_name'][$a] != '' && $input['strategy_name'][$a] != null) {
					$this->model->insert_strategy_detail($postData_detail);
				}
			}
		} catch (Exception $e) {
	...
	public function edit($value)
	{
		...
		$this->pageData["data_strategy_doeb"] = $this->model->getByID_detail($value);
		$this->pageData["count_data_grid"] = count($this->pageData["data_strategy_doeb"]);
	...
	public function update()
	{
		...
		$this->model->update($input['strategy_doeb_id'], $postData);

		$this->model->clear_strategy_detail($input['strategy_doeb_id']);
		$count_strategy_name = count($input['strategy_name']);

		for ($a = 0; $a < $count_strategy_name; $a++) {
			$postData_detail = [
				"strategy_doeb_id" => $input['strategy_doeb_id'],
				"strategy_name" => $input['strategy_name'][$a],
				"remarks" => $input['remarks'][$a],
				"create_by" => '1',
				"create_datetime" => Date("Y-m-d H:i:s"),
	...
	public function delete($value)
	{
		// iclude helper form
		helper("form");
		$this->model->delete_strategy_doeb($value);
		/*
