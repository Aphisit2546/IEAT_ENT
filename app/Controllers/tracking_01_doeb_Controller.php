...
class tracking_01_doeb_Controller extends BaseController
{
	protected $programCode = "tracking_01_doeb";
	...
	public function add($value)
	{
		...
		helper("form");
		$client = new Client();
		$res = $client->put(Env("app.RestBaseURL") . '/api/MonthlyPlanBudgetProjectTrackingApproveDoeb/' . $value, [
	...
	public function edit($value)
	{
		...
		helper("form");
		$client = new Client();
		$res = $client->put(Env("app.RestBaseURL") . '/api/MonthlyPlanBudgetProjectTrackingReturnDoeb/' . $value, [
			"body" => json_encode($value)
		]);
		$data = $res->getBody()->getContents('json');
		$data = json_decode($data, true);
	...
	public function showall()
	{
		...
		$input = $this->request->getRawInput();
		$input['GroupUserID'] = session('GroupUserID');
		$res = $client->request('GET', Env("app.RestBaseURL") . '/api/MonthlyPlanBudgetProjectTrackingAllDoeb', [
			"body" => json_encode($input)
		]);
