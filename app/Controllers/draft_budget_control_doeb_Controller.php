...
class draft_budget_control_doeb_Controller extends BaseController
{
	...
	public function edit($value)
	{
		...
		$this->pageData['type_submonth'] = $split[3];
		...
		$this->pageData['GroupTypeID'] = session('GroupTypeID');
	...
	public function storesub()
	{
		...
		for ($iii = 1; $iii <= $input['count_record']; $iii++) {
			...
			if (!empty($input["AddDataDetail2_" . $iii])) {
				...
				echo "<script type='text/javascript'>parent.window.location='" . base_url() . "draft_budget_control_doeb/edt/$value_next.Y';</script>";
				exit();
			}
		}
	}
	...
	public function edtsub($value)
	{
		...
		$input_ref['DraftBudgetControlDetail_id'] = $split[1];
		$res_ref = $client->request('GET', Env("app.RestBaseURL") . '/api/DraftBudgetControlDetailForSubDoeb', [
			"body" => json_encode($input_ref)
		]);
		...
		$this->pageData["data_ref"] = $data_ref['data'];
	...
	public function edtupdatesub()
	{
		...
		if (!empty($input["btnSaveSub"])) {
			...
			$client->delete(Env("app.RestBaseURL") . '/api/DraftBudgetControlDetailSubDoeb/' . $DraftBudgetControlDetail_id, [
				"body" => json_encode($DraftBudgetControlDetail_id)
			]);

			$seq = 0;
			foreach ($input['NameDetail'] as $NameDetail) {

				if ($NameDetail != '' && $NameDetail != null && $DraftBudgetControlDetail_id != null && $DraftBudgetControlDetail_id != null) {
					$postData_detail_sub = array(
						"NameDetail" => $NameDetail,
						"UnitManagement_id" => $input['UnitManagement_id'][$seq],
						"quantity" => $input['quantity'][$seq],
						"price" => $input['price'][$seq],
						"price_all" => $input['price_all'][$seq],
						"Description" => $input['Description'][$seq],
						"RemarksSub" => $input['RemarksSub'][$seq],
						"DraftBudgetControlDetail_id" => $DraftBudgetControlDetail_id,
						"DraftBudgetControl_id" => $DraftBudgetControl_id,
						'GroupUserID' => session('GroupUserID'),
						'GroupTypeID' => session('GroupTypeID'),
					);
					...
					$client->post(Env("app.RestBaseURL") . '/api/DraftBudgetControlDetailSubDoeb', [
						"body" => json_encode($postData_detail_sub)
					]);
				}
			}
	...
	public function edtsub2($value)
	{
		...
		$this->pageData["DraftBudgetDetailSub"] = $data['data'];
		$this->pageData["data_ref"] = $data_ref['data'];
		return view($this->routeGroup . "/" . $this->routeGroup . "_add_detailsub_month", $this->pageData);
	}

	public function edtupdatesub2()
	{
		error_reporting(0);
		helper("form");
		$input = $this->request->getRawInput();
		$client = new Client();
		if (!empty($input["btnSaveSub2"])) {
			...
			for ($i = 0; $i < count($input['DraftBudgetControlDetailSub_id']); $i++) {

				$client->delete(Env("app.RestBaseURL") . '/api/DraftBudgetControlDetailSubMonthDoeb/' . $input['DraftBudgetControlDetailSub_id'][$i], [
					"body" => json_encode($input['DraftBudgetControlDetailSub_id'][$i])
				]);

				if ($input['DraftBudgetControlDetailSub_id'][$i] != '' && $input['DraftBudgetControlDetailSub_id'][$i] != null) {
					$postData_detail = [
						"month01" => $input['month01'][$i],
						"month02" => $input['month02'][$i],
						"month03" => $input['month03'][$i],
						"month04" => $input['month04'][$i],
						"month05" => $input['month05'][$i],
						"month06" => $input['month06'][$i],
						"month07" => $input['month07'][$i],
						"month08" => $input['month08'][$i],
						"month09" => $input['month09'][$i],
						"month10" => $input['month10'][$i],
						"month11" => $input['month11'][$i],
						"month12" => $input['month12'][$i],

						"DraftBudgetControlDetailSub_id" => $input['DraftBudgetControlDetailSub_id'][$i],
						'GroupUserID' => session('GroupUserID'),
					];
					//print_r($postData_detail);


					$client->post(Env("app.RestBaseURL") . '/api/DraftBudgetControlDetailSubMonthDoeb', [
